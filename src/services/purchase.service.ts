    ...
    try {
      ...
      if (purchaseData.quantity > sale.maxPurchasePerUser) {
        throw new BadRequestError(
          `Cannot purchase more than ${sale.maxPurchasePerUser} units per user`
        );
      }
      ...
      if (userPurchasesCount >= sale.maxPurchasePerUser) {
        throw new BadRequestError(
          "You have already reached the maximum purchase limit for this sale"
        );
      }
      ...
      const updatedSale = await Sale.findOneAndUpdate(
        ...
        },
        {
          ...
          $set: {
            ...
            ...(sale.remainingUnits - purchaseData.quantity <= 0
              ? { endTime: now }
              : {}),
          },
        },
      ...
      if (!updatedSale) {
        throw new BadRequestError(
          "Stock was depleted before your purchase could be processed"
        );
      }
  ...
  public async getSaleLeaderboard(
    ...
    const [leaderboardData, totalCount] = await Promise.all([
      Purchase.aggregate([
        ...
        { $match: { rank: { $gt: skip, $lte: skip + limit } } },
        {
          $lookup: {
            from: "users",
            localField: "userId",
